...
static void showtagpreview(int i);
...
static void positionpreview(Monitor *m, int tag) {
  int pw = m->mw / scalepreview;
  int ph = m->mh / scalepreview;

  int tx = m->wx + m->tagx[tag] + m->tagw[tag] / 2 - pw / 2;
  int ty = m->by + bh;
  ...
  XMoveResizeWindow(dpy, m->tagwin, tx, ty, pw, ph);
}
...
Monitor *createmon(void) {
  ...
  m->tagwin = XCreateWindow(
      dpy, root, 0, 0, /* x, y (will be moved later) */
      1, 1,            /* minimal size */
      0,               /* border width */
...
void drawbar(Monitor *m) {
  ...
  if ((w = m->ww - tw - x) > bh) {
    ...
    if (m->sel) {
      ...
      titlebuf[50] = '\0';
      if (strlen(m->sel->name) > 50)
        strcpy(titlebuf + 47, "...");

      int textw = drw_fontset_getwidth(drw, titlebuf);
      ...
      if (have_icon) {
        ...
        drw_pic(drw, icon_x, icon_y, m->sel->icw, m->sel->ich, m->sel->icon);
        start += iconw + spacing;
      }
...
Picture geticonprop(Window win, unsigned int *picw, unsigned int *pich) {
  ...
  uint32_t icw, ich;
  uint32_t maxh = bh > 2 ? (bh - 2) : bh; /* fit icon to bar height */
...
void manage(Window w, XWindowAttributes *wa) {
  ...
  if (XGetTransientForHint(dpy, w, &trans) && (t = wintoclient(trans))) {
    ...
    /* Detect Polybar by WM_CLASS */
    XClassHint ch = {NULL, NULL};
    XGetClassHint(dpy, c->win, &ch);
    const char *wmclass = ch.res_class ? ch.res_class : "";

    if (strstr(wmclass, "polybar")) {
      c->isfloating = 1;
      c->bw = 0;
      c->neverfocus = 1;
    }
...
void motionnotify(XEvent *e) {
  ...
  /* mouse left tag area → hide preview */
  if (m->previewshow) {
    m->previewshow = 0;
...
void showtagpreview(int tag) {
  if (!selmon->previewshow || !selmon->tagmap[tag]) {
    XUnmapWindow(dpy, selmon->tagwin);
    return;
  }

  positionpreview(selmon, tag);
...
void takepreview(void) {
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    ...
    image = imlib_create_image(selmon->mw, selmon->mh);
    if (!image)
      continue;
...
void tag(const Arg *arg) {
  ...
  if (selmon->sel && (arg->ui & TAGMASK)) {
    ...
    setclienttagprop(c); /* <-- REQUIRED for preserving tags */
    focus(NULL);
    arrange(selmon);
  }
}
...
void togglefloatingandarrange(const Arg *arg) {
  togglefloating(arg);
  arrange(selmon);
}
...
void reload(const Arg *arg) { execvp(argv0, (char *const[]){argv0, NULL}); }
